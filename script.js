	...
	for (var key in moviesObj){
		...
		if (key!=='genres'){
			...
			moviesObj[key].results.forEach(function(movie, index){
				var posterPath = movie.poster_path === null ? 'http://crossorigin.me/https://www.movli.com/images/movie-default.jpg' : 'http://crossorigin.me/https://image.tmdb.org/t/p/w300' + movie.poster_path
...
function createButton(genre){
	...
	$('#buttons').append('<a-entity id='+ genre +' rotation="0 '+ buttonRotation +' 0"><a-curvedimage id='+genre+' src="http://crossorigin.me/https://fakeimg.pl/439x230/282828/eae0d0/?text='+ genre +'"radius=5.7 theta-length=35 height=.85 position="0 '+(-0.535+heightmod)+' 0"scale=".4 .4 .4"opacity=.8><a-mouseenter target=#'+genre+'_glow opacity=1></a-mouseenter><a-mouseleave target=#'+genre+'_glow opacity=.2></a-mouseleave></a-curvedimage><a-curvedimage id='+genre+'_glow src="glow.jpg" rotation="0 1.5 0"radius=5.7 theta-length=38 height=1.1 position="0 '+(-0.67+heightmod+secondRowGlow)+'-0.67 0"scale=".5 .5 .5"opacity=.2></a-curvedimage></a-entity>')
	buttonRotation += 40;
}
					...
					moviesObj[key].results.forEach(function(movie, index){
						for(var prop in movie){
							if (movie[prop] === parseInt(clickedId) && !found) {
								...
								$("#overlay").hide();
								$('#background').css({
									'background' : 'url(http://crossorigin.me/https://image.tmdb.org/t/p/w1280' + movie.backdrop_path+') no-repeat',
